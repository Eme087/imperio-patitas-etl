...
def test_print_single_product_info():
    ...
    # 1. OBTENER PRODUCTO Y SUS VARIANTES
    # Hacemos una sola llamada pidiendo que la API "expanda" las variantes y sus costos.
    # Esto es más eficiente que hacer múltiples llamadas.
    product = bsale_client.get(f"products/{PRODUCT_ID_TO_TEST}.json", params={'expand': '[variants,variants.costs]'})
    # 1. OBTENER PRODUCTO, VARIANTES, COSTOS Y STOCK EN UNA SOLA LLAMADA
    # Usamos 'expand' para traer todas las relaciones y evitar múltiples peticiones.
    # Esto es mucho más eficiente.
    product = bsale_client.get(f"products/{PRODUCT_ID_TO_TEST}.json", params={'expand': '[variants,variants.costs,variants.stock]'})
    if not product:
    ...
    # 3. OBTENER PRECIO, COSTO, STOCK Y SKU
    # El SKU y el Costo ya vienen en el objeto 'active_variant' gracias al 'expand'.
    ...
    cost = active_variant.get('costs', {}).get('averageCost', 0.0)

    # CORRECCIÓN: El costo puede ser None o un string. Lo convertimos a float de forma segura.
    cost_str = active_variant.get('costs', {}).get('averageCost')
    cost = float(cost_str) if cost_str else 0.0

    # OPTIMIZACIÓN: El stock se obtiene directamente de la variante expandida.
    stock = active_variant.get('stock', {}).get('quantityAvailable', 0.0)

    # Para el Precio, consultamos la lista de precios.
    price_details = bsale_client.get(f"price_lists/{PRICE_LIST_ID}/details.json", params={'variantid': variant_id})
    price = price_details['items'][0]['variantValueWithTaxes'] if price_details.get('count', 0) > 0 else 0.0

    # Para el Stock, consultamos el endpoint de stocks.
    stock_details = bsale_client.get("stocks.json", params={'variantid': variant_id})
    stock = stock_details['items'][0]['quantityAvailable'] if stock_details.get('count', 0) > 0 else 0.0
    ...
    print(f"  Costo Promedio:   ${float(cost):,.2f}")
